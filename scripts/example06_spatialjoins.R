...
CC_CCVI_sf <- arc.data2sf(CC_CCVI_df)
...
CC_SVI_sf <- arc.data2sf(CC_SVI_df)
...
CC_CensusTracts_sf <- arc.data2sf(CC_CensusTracts_df)
...
CC_Border_sf <- arc.data2sf(CC_Border_df)

# Transform datasets to same coordinate system, UTM Z16

# CC_ZipCodes_sf <- st_transform(CC_ZipCodes_sf, crs = 26916)

CC_Border_sf <- st_transform(CC_Border_sf, crs = 26916)
CC_Municipalities_sf <- st_transform(CC_Municipalities_sf, crs = 26916)
CC_CensusTracts_sf <- st_transform(CC_CensusTracts_sf, crs = 26916)
CC_CommissionerDistricts_sf <- st_transform(CC_CommissionerDistricts_sf, crs = 26916)
CC_ZipCodes_sf <- st_transform(CC_ZipCodes_sf, crs = 26916)
masterlist_address_sf <- st_transform(masterlist_address_geo,crs = 26916)

CC_CensusTracts_sf_sub <- CC_CensusTracts_sf %>%
...
masterlist_address_sf <- masterlist_address_sf %>% st_join(CC_CensusTracts_sf_sub)
masterlist_address_sf <- masterlist_address_sf %>% st_join(CC_Municipalities_sf_sub)
masterlist_address_sf <- masterlist_address_sf %>% bind_cols(locations)
masterlist_address_sf <- masterlist_address_sf %>% mutate(street = toupper(street), city=toupper(city))

ggplot() + geom_polygon(CC_Border_sf) + geom_point(masterlist_address_sf)
plot(CC_CensusTracts_sf['GEOID10'])

st_write(CC_Municipalities_sf,"CC_Municipalities_sf.shp")
st_write(masterlist_address_sf,"masterlist_address_sf.shp")
st_write(CC_Border_sf,"CC_Border_sf.shp")
st_write(CC_CensusTracts_sf,"CC_CensusTracts_sf.shp")
